...
#penalize paths that go to public transit places as more expensive 
def h(place):
    places = ['Howard_Western','Howard_Ridge','Howard_L','Jarvis_Clark','Jarvis_L','Touhy_Clark','Touhy_Glenwood','Greenleaf_Sheridan','Lunt_Western','Lunt_Clark','Morse_L','Pratt_Ridge','Columbia_Clark','Columbia_Sheridan']
    if not place in places:
        return min(RP[place].values()) #makes places not in this places less expensive
    else:
        return max(RP[place].values())*1000 #makes places that are bus/L stops more expensive
    
#tried doing euclidean but it didn't work
#def h(s,t):
...
def a_star(G,s,t,h):
    P,Q = {}, [(h(s), None,s)] #pred and queue with heurisitic # Preds and queue w/heuristic
    while Q: #unprocessed nodes 
        d,p,u = heappop(Q) #lowest heuristic  
        if u in P: continue #already in pred then skip 
        P[u] = p #set path to pred
        if u == t: return d-h(t), P #arrived
        for v in G[u]:
            w = G[u][v] - h(u) + h(v) #modify weight with heuristic then add queue 
            heappush(Q,(d+w,u,v))
    return inf, None
...
def calc_all():
    ...
    for i in places:
        t = i
        val = a_star(RP,'Home', t, h)
        dist.append(val[0])
    dictionary = dict(zip(places,dist))
    print(dictionary)
    return str(sum(dictionary.values()))
